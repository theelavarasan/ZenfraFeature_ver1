...
public class AwsInventoryController {
	...
	public ResponseModel_v2 saveAws(@RequestBody AwsInventory aws){
		...
		try {
			...
			String query="INSERT INTO aws_cloud_credentials(userid, sitekey, access_key_id, secret_access_key, regions, description, created_date, data_id)" + 
					"VALUES (':userid_value', ':sitekey_value', ':access_key_id_value', ':secret_access_key_value', ':regions_value'::json, ':description_value'," + 
					"':created_date_value', gen_random_uuid())" + 
					...
					",userid=':userid_value',secret_access_key=':secret_access_key_value' ,regions=':regions_value'::json,description=':description_value',updated_date=':updated_date_value'";
			...
			query=query.replace(":userid_value", aws.getUserid()).replace(":sitekey_value", aws.getSitekey()).replace(":access_key_id_value", aws.getAccess_key_id())
					...
					.replace(":created_date_value", aws.getCreated_date()).replace(":updated_date_value", aws.getUpdated_date());
