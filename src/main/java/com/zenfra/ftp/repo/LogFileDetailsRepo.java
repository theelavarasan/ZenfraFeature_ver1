...
public interface LogFileDetailsRepo extends JpaRepository<LogFileDetails, String>{
	...
	@Query(value="select distinct((case when log_type ilike 'tanium' then 'Linux'  when lower(log_type) ilike '%mode%' then 'netapp' else log_type end)) from log_file_details where is_active=:isActive and  site_key=:siteKey and status=:status group by log_type order by log_type" ,nativeQuery = true)
