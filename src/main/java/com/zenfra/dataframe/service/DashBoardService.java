...
public class DashBoardService {
	...
	public Boolean deleteDashboardChart(String chartId, String siteKey, String userId) {
		try {
			...
			String query = queries.dashBoardChart().getGetByChartIdSiteKeyUserId().replace(":user_id", userId)
					.replace(":site_key", siteKey).replace(":chart_id", chartId);
	...
	public JSONObject getDashboardChartDetails(DashboardInputModel dashboardInputModel) {
		...
		try {
			...
			String query = queries.dashboardQueries().getGetDashboardChartDetails()
					.replace(":chart_id", dashboardInputModel.getChartId())
					.replace(":site_key", dashboardInputModel.getSiteKey());
	...
	public JSONObject getChatForFavMenu(String favoriteViewId, String userId, String siteKey) {
		...
		try {
			...
			String getChatForFavMenu = queries.dashboardQueries().getGetChatForFavMenu()
					.replace(":favourite_id", favoriteViewId).replace(":user_id_value", userId)
					.replace(":site_key_value", siteKey);
	...
	private DashboardDataResponseModel getProjectSummaryDetails()
			throws org.json.simple.parser.ParseException, URISyntaxException, ClientProtocolException, IOException {
	...
	public DashBoardCharts getDashChartsByUserIdSiteKey(String userId, String siteKey, String chartId) {
		...
		try {
			...
			String query = queries.dashBoardChart().getGetSiteKeyUserIdChartId().replace(":user_id", userId)
					.replace(":site_key", siteKey).replace(":chart_id", chartId);
			System.out.println(query);
	...
	public DashboardChartDetails getDashboardChartDetailsBySiteKey(String siteKey, String chartId) {
		...
		try {
			...
			String query = queries.dashBoardChartDetails().getGetByChartIdSiteKey().replace(":site_key", siteKey)
	...
	public Object getProjectSummary(DashboardInputModel dashboardInputModel, String token) {
		...
		try {
			...
			Response execute = Jsoup.connect(
					"http://uat.zenfra.co:8080/zenfra-features/rest/dashboard/layout?siteKey=67483b74-b221-4057-be91-c06c4011fa66&tenantId=a0fe5fb8-f660-46fb-9c3d-eae1d3847238&userId=223247ff-9f05-4612-87b1-27e47d427003")
			...
			/*
			 * Response execute = Jsoup.connect(
			 * "http://uat.zenfra.co:8080/ZenfraV2/rest/reports/dashboard/data")
			 ...
			 * .requestBody(dashboardInputModel.toString()) .maxBodySize(1_000_000 * 30) //
			 * 30 mb ~ .timeout(0) // infinite timeout .execute();
			 */
	...
	public JSONObject getChartFavMenuByAnalyticsType(String siteKey, String favouriteId) {
	...
	public DashboardUserCustomization getDashUserCusBySiteKeyAndUserId(String siteKey, String userId) {
		...
		try {
			String query = "select * from dashboard_user_customization where site_key='" + siteKey + "' and user_id='"
					+ userId + "'";
